...
namespace LostAndFound.Application.Features.Items.Commands.Common;

public abstract record CreateItemCommandBase(
    string Title,
    string Description,
    int CategoryId,
    string Location,
...
public abstract class CreateItemCommandValidator<T> : AbstractValidator<T> where T : CreateItemCommandBase
{
    protected CreateItemCommandValidator(ICategoryRepository categoryRepository)
    {
        RuleFor(x => x.Title)
